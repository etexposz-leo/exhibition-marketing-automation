...
function initGenerate() {
    ...
    // Publish buttons (old single platform)
    ...
    document.getElementById('publishAllBtn').addEventListener('click', publishToSelected);
}
...
async function publishToSelected() {
    ...
    let content = currentContent.linkedin;
    const activeTab = document.querySelector('.tab-btn.active');
    if (activeTab) {
        const tab = activeTab.dataset.tab;
        if (tab === 'facebook') content = currentContent.facebook;
        else if (tab === 'google') content = currentContent.google;
    }
    
    if (!content) {
        showToast('No content to publish', true);
        return;
    }
    
    const btn = document.getElementById('publishAllBtn');
    btn.disabled = true;
    btn.innerHTML = '<svg class="spinner" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3" fill="none" stroke-dasharray="30 70"/></svg> Publishing...';
    ...
    try {
        ...
        btn.disabled = false;
        btn.innerHTML = '🚀 Publish to Selected Platforms';
    }
}
